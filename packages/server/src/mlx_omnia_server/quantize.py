...
`~/.cache/mlx_omnia/quantized/<name>/<digest>`: the digest of the plan over the source *is*
...
from mlx_omnia.quant.gptq import gptq, to_affine
...
from mlx_omnia_server.jobs import Job, Jobs, JobsDep, Progress, Quantize, Work, accepted
...
def _install(staged: Path, digest: str, final: Path) -> None:
    ...
    snapshot is the entry, and how `mlx_omnia.load` finds it with no Hub to ask."""
