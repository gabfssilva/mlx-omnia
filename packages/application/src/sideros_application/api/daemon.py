...
from sideros_application.api.http import BASE

# packages/application/src/sideros_application/daemon.py — the checkout is five up, and
# it is where `uv run` has to be pointed when the console script is not on this path.
ROOT = pathlib.Path(os.environ.get("SIDEROS_PROJECT") or pathlib.Path(__file__).parents[4])
...
async def _spawn() -> asyncio.subprocess.Process:
    beside = pathlib.Path(sys.executable).parent / "sideros-server"
    command = (
        [str(beside)]
        if beside.exists()
        else ["uv", "run", "--project", str(ROOT), "sideros-server"]
    )
