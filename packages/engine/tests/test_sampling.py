...
def test_temperature_divides_the_logits() -> None:
    scaled = temperature(2.0)(LOGITS)[0].tolist()
    assert scaled == [1.5, 0.5, 1.25, -0.5, 0.25]


def test_temperature_rejects_zero() -> None:
    # Greedy is the answer at zero, and it is a different function: dividing by it here
    # would hand the sampler a row of infinities.
    with pytest.raises(ValueError, match="temperature must be positive"):
        temperature(0.0)


def test_top_k_keeps_exactly_k_without_ties() -> None:
    assert kept(top_k(2)(LOGITS)) == {0, 2}
    assert kept(top_k(1)(LOGITS)) == {0}
    assert kept(top_k(5)(LOGITS)) == {0, 1, 2, 3, 4}


def test_top_k_keeps_the_whole_block_tied_at_the_kth() -> None:
    tied = mx.array([[3.0, 2.0, 2.0, 1.0]])
    assert kept(top_k(2)(tied)) == {0, 1, 2}


def test_top_k_tolerates_a_k_wider_than_the_vocabulary() -> None:
    assert kept(top_k(50)(LOGITS)) == {0, 1, 2, 3, 4}


def test_top_p_cuts_on_the_accumulated_mass() -> None:
    probs = probabilities(LOGITS)
    order = sorted(range(len(probs)), key=lambda i: probs[i], reverse=True)
    # 0.60, 0.13, 0.08, ... : two tokens carry more than 0.7 of the mass, one does not.
...
def test_top_p_of_one_keeps_everything() -> None:
    assert kept(top_p(1.0)(LOGITS)) == {0, 1, 2, 3, 4}


def test_min_p_is_relative_to_the_most_probable_token() -> None:
    probs = probabilities(LOGITS)
    top = max(probs)
...
def test_filters_run_in_the_order_given() -> None:
...
def test_out_of_range_arguments_are_refused(factory: object, value: float) -> None:
...
def test_a_single_candidate_is_drawn_deterministically() -> None:
    draw = sampler(top_k(1))
    for _ in range(16):
        assert draw(LOGITS).tolist() == greedy(LOGITS).tolist()


def test_the_draw_never_leaves_the_kept_set() -> None:
    draw = sampler(top_k(2), seed=7)
    drawn = {int(draw(LOGITS).item()) for _ in range(64)}
    assert drawn <= {0, 2}
    assert len(drawn) == 2, "a 64-draw sample that never picks the second candidate is suspect"


def test_the_same_seed_replays_the_same_sequence() -> None:
    flat = mx.zeros((1, 64))
...
def test_an_unseeded_sampler_advances_on_its_own() -> None:
    flat = mx.zeros((1, 64))
    draw = sampler()
...
def test_the_penalty_moves_seen_ids_towards_zero_from_both_sides() -> None:
    logits = mx.array([[2.0, -2.0, 1.0]])
    adjusted = repetition_penalty(2.0)(logits, mx.array([0, 1]))[0].tolist()
    assert adjusted == [1.0, -4.0, 1.0]


def test_the_penalty_only_reads_its_context_window() -> None:
...
def test_the_penalty_leaves_an_empty_history_alone() -> None:
    logits = mx.array([[2.0, -2.0, 1.0]])
    empty = mx.array([], dtype=mx.int32)
    assert repetition_penalty(2.0)(logits, empty)[0].tolist() == logits[0].tolist()


def test_a_penalty_of_one_is_the_identity() -> None:
    logits = mx.array([[2.0, -2.0, 1.0]])
    assert repetition_penalty(1.0)(logits, mx.array([0, 1, 2]))[0].tolist() == logits[0].tolist()


def test_a_non_positive_penalty_is_refused() -> None:
    with pytest.raises(ValueError, match="repetition_penalty must be positive"):
        repetition_penalty(0.0)


# ── the penalty inside the loop ──────────────────────────────────────────────
...
def test_without_a_penalty_the_constant_model_repeats_itself() -> None:
    assert list(stream_ids(DESCENDING, [4], max_tokens=4)) == [0, 0, 0, 0]


def test_the_penalty_walks_down_the_ranking() -> None:
    # Each drawn id enters the history and drops below the next one, so the run reads
    # off the ranking in order — which only holds if the loop feeds back the id it drew.
    ids = list(stream_ids(DESCENDING, [4], max_tokens=4, penalty=repetition_penalty(10.0)))
    assert ids == [0, 1, 2, 3]


def test_the_prompt_counts_as_history() -> None:
    # id 0 is already in the prompt, so the first draw skips it.
    ids = list(stream_ids(DESCENDING, [0], max_tokens=3, penalty=repetition_penalty(10.0)))
    assert ids == [1, 2, 3]


def test_the_window_slides() -> None:
    # With room for two ids, the third draw finds id 0 out of the window and eligible.
    penalty = repetition_penalty(10.0, context=2)
    ids = list(stream_ids(ConstantLM([5.0, 4.0, 3.0]), [2], max_tokens=6, penalty=penalty))
