...
stable descending sort by `score + bias` and the opposite of what `softmax_topk`'s
...
`ORDINAL_HEADER` is shared with `route.residual`, which emits the same keys straight
...
from sideros.core.kernels.route.kernel import Routing, gate_logits
...
class OrdinalRoute:
    ...
    on `score + bias`, not the higher-index `simd_max` scan the other kernels run.
    """
    ...
    def build(cls, gate: mx.array | None, *, routing: Routing) -> Self | None:
        if routing.scoring != "sigmoid" or routing.bias is None:
            return None
        if not routing.normalize or routing.norm_eps != 0.0 or routing.scale != 1.0:
            return None
        if routing.shared or routing.hash_table is not None or routing.softcap != 0.0:
            return None
        if not router_tournament_applies(routing.experts, routing.k):
            return None
    ...
    def __call__(
        ...
        *,
        ...
        return router_tournament(gate_logits(self.gate, row, logits), bias, self.routing.k)
