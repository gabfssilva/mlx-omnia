...
from mlx_omnia.core.kernels.mlp.kernel import Activation
...
class DenseMlp:
    ...
    def build(
        ...
        # Both kernels bake silu and name bf16 in their loads, their epilogue and
        # their stores; a quantized leaf has no dense rows to read.
        ...
        if not isinstance(gate_up, nn.Linear) or not isinstance(down, nn.Linear):
            return None
        if not dense_gate_up_swiglu_applies(hidden, inner, gate_up.weight.dtype):
            return None
        if not dense_down_residual_applies(hidden, inner, down.weight.dtype):
            return None
        return cls(gate_up.weight, down.weight)

    def __call__(self, row: mx.array, residual: mx.array) -> mx.array:
        activated = dense_gate_up_swiglu(row, self.fused_weight)
        return dense_down_residual(activated, self.down_weight, residual)
