...
The sibling of `attention.sliding` for a layer that attends to everything written
...
from mlx_omnia.core.cache import FixedKVCache
from mlx_omnia.core.kernels.attention.default import DefaultAttentionStep
...
class FullAttentionStep:
    ...
    nothing else exists to mask. Prefill (`T != 1`) is not this kernel's shape and runs on
    the ops chain, which writes the same rows.
    """
    ...
    def build(
        ...
        *,
        ...
        if not isinstance(cache, FixedKVCache) or angles is None or sinks is not None:
            return None
        ...
        if heads % 2 != 0 or heads % kv_heads != 0 or (heads // kv_heads) % 2 != 0:
            return None
        quarter = rotary_pairs // 4
        if rotary_pairs == 0 or rotary_pairs % 4 != 0 or quarter & (quarter - 1) != 0:
            return None
        if 2 * rotary_pairs > head_dim:
            return None
        keys, values = cache.fetch()
        if keys.ndim != 4 or keys.shape[:2] != (1, kv_heads) or keys.shape[3] != head_dim:
            return None
