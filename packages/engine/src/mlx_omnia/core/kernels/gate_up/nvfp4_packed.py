...
is *recomputed* from the router's ordinal keys — which is what the `OrdinalRouting`
declaration says the second `__call__` argument carries. Each lane holds `experts / 32` of
them (expert `lane + 32j`), and one round of `router_topk_extract_round` is an argmin over
that register file followed by a five-step butterfly — the (ordinal, index) pair travels as
a `uint2` so both components come from the same source lane — leaving every lane holding
the same winner, which the owning lane then masks out of its register. Threadgroup `s` runs
...
Two orderings meet in the weight addressing. The kernel walks a bank that interleaves gate
and up in tiles of 32 rows (`gate_row = (row / 32) * 64 + row % 32`, up at `+32`), so the
leaf's own row interleave is permuted into it once, at build; the scale plane is a side
copy already permuted into the kernel's walk order — per expert `[tile of 4 rows][k-block]
[sub][16 bytes]`, `sub` being the four rows times gate/up — so a lane's scale byte is one
address computed from `logical_row / 4` and `logical_row % 4`, and adjacent k-blocks are
128 bytes apart instead of a row apart.
...
iteration are already in flight while the ALU works. `shared.nvfp4` covers the decode and
...
from mlx_omnia.core.kernels.route.ordinal import ORDINAL_HEADER
...
def tile_row_order(rows: int) -> mx.array:
    """Row permutation from the leaf's row-interleaved bank (`[g0, u0, g1, u1, ...]`) to
    the one the kernel walks: tiles of 32 rows, `gate_row = (row / 32) * 64 + row % 32`
    with up at `+32`. The bank is reordered once, at build, rather than the kernel bent to
    fit."""
    logical = rows // 2
    order = [0] * rows
...
def pack_gate_up_scales(fused_scales: mx.array) -> mx.array | None:
    """The fused gate/up scale plane `[experts, 2*inner, hidden/16]`, already in the
    kernel's tile order, as the bank the kernel addresses: bytes are reordered, never
    recomputed.

    Per expert the walk order is `[tile of 4 logical rows][k-block][sub][16 bytes]` with
    `sub = (logical_row % 4) * 2 + {0 gate, 1 up}`, which bakes the bank's 32-row gate/up
    interleave into scale storage order. The halving then collapses each 32-byte row-block
    to the 16 even bytes; the walk order puts expert 0's gate row 0 at row-block 0 and its
    up row 0 at row-block 1, so the two spans the quantizer may leave unequal are flat
    pairs 0 and 16 and land in header slots 0 and 1 — where the kernel's patch lane reads
    them. `None` when the checkpoint fails that certificate."""
...
def applies(hidden: int, inner: int, experts: int, top_k: int) -> bool:
    """A lane covers 16 values and the K loop steps a whole 512-value block, so the
    contraction is blocked, not guarded: `hidden` must be a multiple of 512. The bank's
    gate/up interleave is 32 rows wide and the scale tile is four rows, so `inner` must be
    a multiple of 32. The tournament gives each lane `experts / 32` keys and one mask bit
    each, so `experts` is a multiple of 32 and at most 1024, and a threadgroup exists per
    selected slot."""
    return (
        hidden % 512 == 0
        and inner % 32 == 0
    ...
    def build(
        ...
        *,
        ...
        if shared is not None:
            return None
        # Routing lives inside the kernel: without the declaration there are no keys to
        # give it, and the epilogue is the plain SwiGLU chain.
        if routing is None or layout != "interleaved":
            return None
        ...
        if activation != "silu" or limit is not None or bias is not None:
            return None
        experts = leaf.weight.shape[0]
        if not applies(hidden, inner, experts, routing.top_k):
            return None
        order = tile_row_order(leaf.scales.shape[1])
        weight = mx.contiguous(mx.take(leaf.weight, order, axis=1))
        packed = pack_gate_up_scales(mx.take(leaf.scales, order, axis=1))
        if packed is None:
            return None
        mx.eval(weight, packed)
        return cls(weight, packed, routing.top_k)

    def __call__(self, row: mx.array, chosen: mx.array) -> mx.array:
        """`chosen` is the router's ordinal keys [experts] uint32, not expert indices:
        slot `s` of the output holds the `s`-th best expert."""
