...
from mlx_omnia.core.kernels.lm_head.kernel import HeadProjection
...
def lm_head_argmax_applies(
    ...
    *,
    ...
    """Whether the six-kernel chain may serve the greedy step for this geometry.

    The old `argmax_only` gate is gone: the strategy's contract is the greedy step, so a
    caller has no way to reach a non-winning slot and no assertion left to make. What
    remains is geometry -- one row at a time (a fused single-token head, so no batch and no
    prefill block), bf16 on both sides (the exact pass is a literal transcription of mlx's
    bf16 gemv), a hidden size that is a whole number of 1024-element lane passes, and a
    vocabulary that tiles the coarse (16 rows), exact (32 rows) and argmax (128 partitions
    of 4 reads per lane) launches at once -- which is `vocab % 512 == 0` -- with the second
    argmax stage still fitting one simdgroup's worth of partitions.
    """
...
def _assert_chain(vocab: int, hidden: int, dtype: mx.Dtype) -> None:
    assert lm_head_argmax_applies(vocab, hidden, rows=1, dtype=dtype)
...
class ArgmaxGreedyHead:
    ...
    def build(
        ...
        *,
        ...
        """Declines without a dense bf16 head weight, on a geometry the launches cannot
        tile, or when the planar int5 copy fails its `|q| <= 15` guard on this tensor."""
