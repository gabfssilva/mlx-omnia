...
from mlx_omnia.models.apertus.layers.block import ApertusBlock


class ApertusTrunk(nn.Module):
    def __init__(self, config: ApertusConfig) -> None:
        super().__init__()
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size)
        self.layers = [ApertusBlock(config) for _ in range(config.num_hidden_layers)]
        self.norm = nn.RMSNorm(config.hidden_size, eps=config.rms_norm_eps)


class ApertusActivations(NamedTuple):
    embeddings: mx.array
...
class Apertus(nn.Module):
    def __init__(self, config: ApertusConfig) -> None:
        super().__init__()
        self.config = config
        self.model = ApertusTrunk(config)
    ...
    def activations(self, ids: mx.array, cache: list[KVCache] | None = None) -> ApertusActivations:
