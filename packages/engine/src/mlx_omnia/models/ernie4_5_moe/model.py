...
from mlx_omnia.models.ernie4_5_moe.config import Ernie45MoEConfig
from mlx_omnia.models.ernie4_5_moe.layers.block import Ernie45MoEBlock


class Ernie45MoEActivations(NamedTuple):
    embeddings: mx.array
...
class Ernie45MoETrunk(nn.Module):
    def __init__(self, config: Ernie45MoEConfig) -> None:
        super().__init__()
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size)
        self.layers = [Ernie45MoEBlock(config, routes) for routes in config.routes]
...
class Ernie45MoE(nn.Module):
    def __init__(self, config: Ernie45MoEConfig) -> None:
        super().__init__()
        self.config = config
        self.model = Ernie45MoETrunk(config)
