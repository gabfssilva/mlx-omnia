import math
from typing import NamedTuple

import mlx.core as mx
import mlx.nn as nn

from mlx_omnia.core.cache import KVCache
from mlx_omnia.models.gemma2.config import Gemma2Config
from mlx_omnia.models.gemma2.layers.attention import softcap
from mlx_omnia.models.gemma2.layers.block import Gemma2Block


class Gemma2Activations(NamedTuple):
    embeddings: mx.array
    blocks: list[mx.array]
    norm: mx.array
    logits: mx.array


class Gemma2Trunk(nn.Module):
    def __init__(self, config: Gemma2Config) -> None:
        super().__init__()
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size)
        self.layers = [Gemma2Block(config, layer_type) for layer_type in config.attention_types]
        self.norm = nn.RMSNorm(config.hidden_size, eps=config.rms_norm_eps)


class Gemma2(nn.Module):
    def __init__(self, config: Gemma2Config) -> None:
        super().__init__()
        self.config = config
        self.model = Gemma2Trunk(config)

    def make_cache(self) -> list[KVCache]:
        return [KVCache() for _ in self.model.layers]

    def embed(self, ids: mx.array) -> mx.array:
        embedded = self.model.embed_tokens(ids)
        scale = mx.array(math.sqrt(self.config.hidden_size), mx.float32).astype(embedded.dtype)
        return embedded * scale

    def head(self, normed: mx.array) -> mx.array:
        return softcap(self.model.embed_tokens.as_linear(normed), self.config.final_cap)

    def activations(self, ids: mx.array, cache: list[KVCache] | None = None) -> Gemma2Activations:
        cache = cache if cache is not None else self.make_cache()
        x = self.embed(ids)
        embeddings = x
        blocks: list[mx.array] = []
        for block, layer_cache in zip(self.model.layers, cache, strict=True):
            x = block(x, layer_cache)
            blocks.append(x)
        normed = self.model.norm(x)
        return Gemma2Activations(embeddings, blocks, normed, self.head(normed))

    def __call__(self, ids: mx.array, cache: list[KVCache] | None = None) -> mx.array:
        return self.activations(ids, cache).logits
