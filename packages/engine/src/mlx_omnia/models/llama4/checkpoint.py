...
from mlx_omnia.models.llama4.config import Llama4Config
from mlx_omnia.models.llama4.model import Llama4
...
def _rename(weights: dict[str, mx.array]) -> dict[str, mx.array]:
    ...
    rename `feed_forward` → `mlp` (Omnia convention)."""
...
def _composite(directory: Path, model: Llama4) -> LanguageModel[ModelInput]:
    ...
    if not tokenizer_path.is_file():
        raise FileNotFoundError(
            "Llama4 uses a tiktoken/o200k tokenizer; mlx_omnia has no tiktoken reader yet. "
