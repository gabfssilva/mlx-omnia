...
from mlx_omnia.models.glm4.layers.block import Glm4Block


class Glm4Trunk(nn.Module):
    def __init__(self, config: Glm4Config) -> None:
        super().__init__()
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size)
        self.layers = [Glm4Block(config) for _ in range(config.num_hidden_layers)]
        self.norm = nn.RMSNorm(config.hidden_size, eps=config.rms_norm_eps)


class Glm4Activations(NamedTuple):
    embeddings: mx.array
...
class Glm4(nn.Module):
    ...
    def activations(self, ids: mx.array, cache: list[KVCache] | None = None) -> Glm4Activations:
