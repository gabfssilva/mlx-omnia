...
from sideros_server.catalog import CatalogEntry
from sideros_server.engine import Engine
from sideros_server.jobs import Cancelled, Job, Progress
from sideros_server.store import PageCache, RunState, SpeedResult, StreamSource
...
class SpeedShape:
    ...
    def key(self) -> str:
        ...
        return (
            ...
            f" · r{self.rounds}+{self.warmup} · {self.sampling.key}"
...
def measure(
    ...
    total = shape.rounds + shape.warmup
    kept: list[Round] = []
    for index in range(total):
        warming = index < shape.warmup
        stage = "warm-up" if warming else "round"
        number = index + 1 if warming else index + 1 - shape.warmup
        outof = shape.warmup if warming else shape.rounds
        job.report(frames.frame(f"{stage} {number} of {outof}"))
        ...
        measured = _drain(
            job, engine, model, prompt, shape.generate, reservation, shape.sampling
        )
        if index >= shape.warmup:
