...
def _estimate(entry: Planned, store: Store) -> float | None:
    ...
    if entry.shape is None or not isinstance(entry.body, SpeedResult):
        return None
    rounds = entry.shape.rounds + entry.shape.warmup
    tokens = rounds * entry.shape.generate * entry.shape.concurrency
    history = [
