"""GET/PATCH /admin/config: the eight parameters the Server screen edits.
...
class Config(BaseModel):
    """The eight and their bounds. This is also where a PATCH is judged: reading a row is a
    ...
    prefix_cache_bytes: int = Field(default=1024**3, ge=0)
    """How much each resident model may keep of the prefixes it has already read, 0 for
    none. A gibibyte is a choice and not a measurement: a 30B MoE's KV is ~98 KB per token,
    so it is about ten thousand tokens of conversation kept per model, and it is counted
    inside `memory_limit_bytes` because the trie's arrays are live allocations the memory
    rail already reads."""
