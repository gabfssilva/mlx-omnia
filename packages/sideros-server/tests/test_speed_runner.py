...
def test_the_key_names_the_shape_and_never_the_model() -> None:
    ...
    assert shape.key == "4k→256 · 4 streams · r3+1 · greedy · warm · queue"
...
def test_the_same_shape_written_two_ways_is_one_key() -> None:
    ...
    second = SpeedShape(context=4096, generate=256, concurrency=1, rounds=3, warmup=1)
...
def test_a_shape_produces_four_measurements_and_drops_the_warm_ups(
    stand: tuple[Engine, Job],
) -> None:
    engine, job = stand
    shape = SpeedShape(context=512, generate=64, concurrency=1, rounds=4, warmup=2)
    ...
    # The script's first three generations are the load probe and the two warm-ups; the
    # four kept rounds are 102, 103, 104 and 105 tok/s, whose median is 103.5.
    assert taken.result.decode_tps == pytest.approx(103.5)
...
def test_the_recorded_ceiling_is_the_one_the_fraction_divides_by(
    ...
    engine, job = stand
    shape = SpeedShape(context=512, generate=64, concurrency=1, rounds=2, warmup=1)
...
def test_a_cold_page_cache_that_cannot_be_produced_is_not_reported_as_cold(
    ...
    shape = SpeedShape(
        context=512, generate=64, concurrency=1, rounds=1, warmup=0, page_cache="cold"
    )
