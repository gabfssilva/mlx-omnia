"""The `sideros` command: an HTTP client, and the evidence that the engine is the daemon's.

Nothing here imports the engine — the package depends on httpx and its transitives and
nothing else, which is what `uv tree --package sideros-cli` is asked to show.
...
`sideros run -m X | jq`, and that is the whole difference between this and a script.
...
from sideros_cli.client import (
    Message,
    ServerError,
    catalog,
    delete_model,
...
def _status(url: str) -> int:
    ...
    print(f"daemon    up at {url} · sideros {machine.version}")
    print(
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="sideros", description="client for the sideros daemon")
