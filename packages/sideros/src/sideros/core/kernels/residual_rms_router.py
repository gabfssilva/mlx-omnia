"""Residual add + rms_norm, and the same fused with the router gemv and its sort keys.

`residual_rms_norm` is the plain pair: one threadgroup per row, four contiguous elements
per thread, the sum of squares reduced through `simd_sum` and a 32-slot threadgroup
array. It overlaps `add_rms_norm` in purpose but not in arithmetic — it walks many rows
per dispatch, reduces with `metal::precise::rsqrt`, and rounds the normed value to `T`
*before* the norm weight multiplies it, where `add_rms_norm` keeps the product in fp32
and rounds once. Neither rounding is a refinement of the other; they are two different
numerical paths and each is pinned by its own test.

`residual_rms_router` is the decode-shaped fusion: one token, and the router gemv
folded into the same dispatch that produced the vector it reads. The normed row is left
in threadgroup memory, so `experts` dot products of length `hidden` are served from there
instead of from a second pass over device memory, and the router logits, their sigmoid
scores and the ordinal sort keys (`router_ordinal`'s) all leave the kernel with the
...
def residual_rms_norm_applies(hidden: int) -> bool:
...
def residual_rms_norm(
    ...
    """The residual join for a block of rows: returns `(residual + branch, its rms_norm)`.

    `residual` and `branch` are `(..., hidden)` of the same shape and dtype; both outputs
    keep it.
    """
    hidden = weight.size
    assert residual.shape == branch.shape and residual.shape[-1] == hidden
    assert residual.dtype == branch.dtype == weight.dtype
    assert residual_rms_norm_applies(hidden)
    rows = residual.size // hidden
    threads = hidden // _N_READS
    out = _NORM_KERNEL(
        inputs=[residual, branch, weight, mx.array(eps, dtype=mx.float32)],
        template=[("T", residual.dtype), ("AXIS", hidden)],
