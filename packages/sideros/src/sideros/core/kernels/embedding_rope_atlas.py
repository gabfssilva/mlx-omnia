...
sliding-window one) whose angle rows have different widths; the kernel copies whichever
prefix each output needs and the launch covers the widest of the three copies. Both
widths are template parameters, and a caller with a single atlas passes the same array
twice or ignores the second output.

The atlas itself -- `[positions, width]` fp32, precomputed once -- is the caller's, not
this kernel's: it is what makes the selection a copy instead of a `cos`/`sin` pair, and
...
def embedding_rope_atlas_applies(
    hidden: int,
    full_width: int,
    sliding_width: int,
    *,
    ...
    of 4. One token per dispatch (`tokens[0]` is the only element read), and the embedding
    is bf16 while the atlases are fp32 -- the kernel names all three types."""
    return (
        rows == 1
...
def embedding_rope_atlas(
    ...
    """The token's embedding row plus both atlases' rows at `position`.

    `tokens` holds the single decoded id, `embedding_weight` is `[vocab, hidden]` bf16, and
    the atlases are `[..., positions, width]` fp32 -> `(hidden [hidden], full_angles
    [full_width], sliding_angles [sliding_width])`.
    """
    hidden = embedding_weight.shape[-1]
    full_width = full_atlas.shape[-1]
    sliding_width = sliding_atlas.shape[-1]
    assert embedding_rope_atlas_applies(
        hidden, full_width, sliding_width, rows=tokens.size, dtype=embedding_weight.dtype
    )
    assert full_atlas.dtype == mx.float32 and sliding_atlas.dtype == mx.float32
    assert 0 <= position < full_atlas.shape[-2] and position < sliding_atlas.shape[-2]
