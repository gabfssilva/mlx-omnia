...
The scale bank is `nvfp4_lane_major`: one base byte and a 4-bit offset per group, in
lane order, so a lane's four blocks arrive as one `ushort`. That single 2-byte load is
what pins the contraction to four 512-value blocks -- four nibbles is all a `ushort`
holds -- and `nvfp4_qmv_applies` says so. Rows the bank could not fit carry base `0xFF`
and take the else arm, which reads the stock plane at the stock stride.
"""

import mlx.core as mx

from sideros.core.kernels.nvfp4_lane_major import LaneMajorScales
