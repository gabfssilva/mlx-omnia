...
class HunyuanDenseAttention(nn.Module):
    def __init__(self, config: HunyuanDenseConfig) -> None:
        ...
        self.qkv_proj = nn.Linear(
            hidden, queries + 2 * key_values, bias=config.attention_bias
        )
        self.o_proj = nn.Linear(queries, hidden, bias=config.attention_bias)
    ...
    def __call__(self, x: mx.array, cache: KVCache) -> mx.array:
        ...
        keys, values = cache.update_and_fetch(self.rope(self.key_layernorm(k), offset), v)
        attended = mx.fast.scaled_dot_product_attention(
            self.rope(self.query_layernorm(q), offset), keys, values,
