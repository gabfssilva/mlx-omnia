...
class Exaone4Attention(nn.Module):
    def __init__(self, config: Exaone4Config, local: bool) -> None:
        ...
        self.rotary = config.rotary or local
        self.window = config.sliding_window if local else None
        self._freqs = (
            None
            if config.rope_scaling is None
            else llama3_freqs(self.head_dim, config.rope_theta, config.rope_scaling.llama3)
        )
