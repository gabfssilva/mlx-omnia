...
class BailingMoEAttention(nn.Module):
    def __init__(self, config: BailingMoEConfig) -> None:
        ...
        self.query_key_value = nn.Linear(
            hidden, queries + 2 * key_values, bias=config.use_qkv_bias
        )
        self.dense = nn.Linear(queries, hidden, bias=config.use_bias)
    ...
    def __call__(self, x: mx.array, cache: KVCache) -> mx.array:
        ...
        offset = cache.offset
        q, k, v = split_qkv(
            self.query_key_value(x),
            heads=self.heads,
            kv_heads=self.kv_heads,
            head_dim=self.head_dim,
        )
        if "query_layernorm" in self:
