...
class GenerationOptions:
    ...
    prefix_budget: int = field(default=0, compare=False)
    """How many bytes of prefix cache this model may keep between requests, 0 for none.

    It travels here for the reason the meter does — out where a caller stands there is a
    conversation and not yet a prompt — and it is a number rather than the cache itself
...
def prefix_cache[C: LayerCache](
    prefix: PromptCache[C] | None, budget: int
) -> PromptCache[C] | None:
    ...
    if budget <= 0:
        return None
    if prefix is not None and prefix.budget == budget:
        return prefix
    return PromptCache(budget)
