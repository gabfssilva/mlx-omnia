...
class RecurrentLM:
    ...
    def __call__(self, ids: mx.array, cache: list[DeltaCache] | None = None) -> mx.array:
        ...
        row = -mx.abs(mx.arange(8) - 3).astype(mx.float32)
        return mx.broadcast_to(row, (1, ids.shape[1], 8))
...
def test_a_trunk_that_cannot_rewind_is_refused_the_trie_instead_of_filling_it() -> None:
    """Declared refusal: a recurrent state is not a history to cut at a common prefix, and
    the answer to that is no reuse — not a rewind that returns a state which never existed.
    The request still runs."""
    trie = PromptCache[DeltaCache](budget=1 << 30)

    ids = list(stream_ids(RecurrentLM(), [1, 2, 3], max_tokens=4, prefix=trie))
