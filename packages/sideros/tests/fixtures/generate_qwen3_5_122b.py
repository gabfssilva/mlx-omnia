...
mlx-lm (git main) over the 4-bit Qwen3.5-122B-A10B checkpoint.

No transformers ground truth exists at this size (fp32 of 122B is ~500GB), so the
reference is mlx-lm over the same packed weights. The DeltaNet, the gated attention
and the sparse MLP are already pinned by the 27B / 35B fixtures; what is new here is
scale — 48 layers, 64 value heads, a 4:1 key-head broadcast, 1024-wide experts, a
144MB recurrent state — so the same tensors are captured against the larger config.
...
- `noise.block_i`: the same two, per block. The residual grows down a 48-layer trunk;
...
Run: MLX_ENABLE_TF32=0 uv run --with git+https://github.com/ml-explore/mlx-lm \
     ...
     packages/sideros/tests/fixtures/generate_qwen3_5_122b.py
...
def main() -> None:
    ...
    path = pathlib.Path(__file__).parent / "qwen3_5_122b_mlxlm.safetensors"
