"""Golden logits, per-block states and greedy ids from mlx-lm over openai/gpt-oss-120b.

Config-only instance of the gpt_oss architecture (128 experts vs 32, 36 layers vs 24);
the model tree, both kernels and the loader are unchanged. No transformers ground
truth exists at this size (no MXFP4 path on Apple Silicon, and a full fp32 forward is
~480 GB). mlx-lm (git main) over the *same packed weights* is the reference, and the
golden is its "exact" forward — the same packed MXFP4 weights with everything else in
float32. That stays feasible at ~70 GB because the MXFP4 expert weights (~61 GB) are
not upcast; only the dense parts (attention, embed, lm_head, norms, router) double.

Two independent bf16 implementations of one graph each round away from that by the
floor, so comparing them to each other would admit twice what comparing to the exact
forward admits.

Floors carried in the fixture, all measured here: `noise.logits` (bf16 against the
fp32-dense graph), `noise.block_i` (the same, per block — the residual grows along a
36-layer trunk, so a single floor is vacuous at one end and impossible at the other)
and `noise.batching` (prefill against step-by-step in mlx-lm; in bf16 an N-row matmul
does not round like N 1-row matmuls).
...
Run: MLX_ENABLE_TF32=0 uv run --no-project \
     ...
     packages/sideros/tests/fixtures/generate_gpt_oss_120b.py
"""
...
def main() -> None:
    ...
    # else in float32. Feasible at ~70 GB because the MXFP4 experts (~61 GB) are not
    # upcast — only the dense parts double. The gap to the bf16 run is what rounding
    # alone costs.
    ...
    save_file(payload, str(fixtures / "gpt_oss_120b_mlxlm.safetensors"))
