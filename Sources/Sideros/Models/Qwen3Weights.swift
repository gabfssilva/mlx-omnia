...
public struct Qwen3Config: Decodable, Sendable {
    public let numHiddenLayers: Int
    public let numAttentionHeads: Int
    public let numKeyValueHeads: Int
    /// Explicit, unlike Qwen2: heads * headDim need not equal hiddenSize (0.6B: 2048 vs 1024).
    ...
    /// The small sizes tie lm_head to the embeddings; 14B and up carry their own.
...
public func loadQwen3Parameters(
    directory: URL, config: Qwen3Config, precision: Precision?, quantization: Quantization? = nil
    ...
    // Tied checkpoints may still serialize lm_head; transformers ties over it, so must we.
    ...
    if let quantization {
        parameters = quantizing(parameters, with: quantization) { name in
            name == "model.embed_tokens.weight" || name == "lm_head.weight"
                || name.hasSuffix("proj.weight")
        }
    }
    return parameters
}

private func expectedShapes(_ config: Qwen3Config) -> [String: [Int]] {
    ...
    for layer in 0..<config.numHiddenLayers {
        ...
        // No attention bias, and q/k are normed per head between projection and rotation.
