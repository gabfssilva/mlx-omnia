...
class PrefixCacheFile:
    ...
    created_at: float
    used_at: float
    """Moved on every hit: the LRU is about use and not about age, and a conversation someone
    keeps coming back to is exactly the one worth the disk."""
...
# The warm-up stopped being part of the shape: the batch runs one per checkpoint, so there is
# no per-shape number to record and none to compare two rows by. The rows written before this
# lose the column and keep everything else — the key they were filed under changes too
# (`r3+1` became `r3`), so they answer as their own history and not as an older answer to a
# question still being asked. The view is dropped and rebuilt because it names the column.
_SCHEMA_V6 = """
DROP VIEW speed_runs;

ALTER TABLE benchmark_speed DROP COLUMN warmup;

CREATE VIEW speed_runs AS
SELECT r.id, r.kind, r.model, r.key, r.state, r.reason, r.engine_version, r.mlx_version,
       r.temp_c_start, r.residents, r.created_at,
       s.context, s.generate, s.concurrency, s.rounds, s.stream_source,
       s.page_cache, s.gate_c, s.load_s, s.prefill_tps, s.ttft_p50_ms, s.ttft_p95_ms,
       s.decode_tps, s.decode_per_stream_tps, s.step_weight_bytes, s.step_kv_bytes,
       s.ceiling_tps, s.ceiling_fraction, s.per_round
FROM benchmark_runs r JOIN benchmark_speed s ON s.run_id = r.id;
...
# The index over the prefix caches spilled to disk. The row is the index and the weight is a
# file under `~/.cache/mlx_omnia/prefixes/`, for the reason the reference cache gives: hundreds
# of megabytes a conversation would bloat the one file a user backs up.
#
# `key` is the digest of model, checkpoint stamp, KV policy and the exact ids — everything
# that changes what the bytes mean and none of which is visible in the file. `model` is beside
# it, denormalised, so "forget this model's conversations" is one DELETE and one rmtree rather
# than a scan that has to open every file to find out whose it is. `used_at` is what the LRU
# reads, and it moves on a hit, which is what makes it a use and not a creation.
_SCHEMA_V7 = """
...
def _speed(row: tuple[object, ...]) -> SpeedResult:
    return SpeedResult(
        ...
        stream_source=_stream_source(row[4]),
        page_cache=_page_cache(row[5]),
        gate_c=_optional_real(row[6]),
        load_s=_optional_real(row[7]),
        prefill_tps=_optional_real(row[8]),
        ttft_p50_ms=_optional_real(row[9]),
        ttft_p95_ms=_optional_real(row[10]),
        decode_tps=_optional_real(row[11]),
        decode_per_stream_tps=_optional_real(row[12]),
        step_weight_bytes=_optional_integer(row[13]),
        step_kv_bytes=_optional_integer(row[14]),
        ceiling_tps=_optional_real(row[15]),
        ceiling_fraction=_optional_real(row[16]),
        per_round=_text(row[17]),
    )
...
_SPEED_COLUMNS = (
    "context, generate, concurrency, rounds, stream_source, page_cache, gate_c,"
    ...
    def prefix_bytes(self) -> int:
        with self._connect() as connection:
            total = connection.execute(
                "SELECT COALESCE(SUM(bytes), 0) FROM prefix_cache"
            ).fetchone()
        return _integer(total[0])

    def prefix_files_by_use(self) -> list[PrefixCacheFile]:
        """Least recently used first, which is the order a ceiling evicts in."""
        with self._connect() as connection:
            rows: list[tuple[object, ...]] = connection.execute(
                f"SELECT {_PREFIX_COLUMNS} FROM prefix_cache ORDER BY used_at, key"
            ).fetchall()
        return [_prefix(row) for row in rows]
