...
The op order matches `DefaultHyperConnection` exactly: `softmax(...) + eps` with
no eps in the denominator, then a column normalization, then `iters - 1` row/column
rounds, every normalization guarded by `+ eps`.

`expand` is the junction's other half — `post * x` broadcast over the copies plus
...
def _applies(hc_mult: int, hidden: int) -> bool:
...
class FusedHyperConnection(HyperConnectionStrategy):
    ...
    def build(
        ...
        # One float4 per comb row and a four-way unrolled collapse: `hc_mult` is exactly
        # 4; the collapse reads `vec<T, 4>`, so the hidden size is a multiple of 4.
        if not _applies(hc_mult, hidden):
            return None
