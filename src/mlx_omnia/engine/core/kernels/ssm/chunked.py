"""Mamba2 SSD: the chunked prefill scan plus the free-function dispatch.

The per-token Metal kernel lives in ``core/kernels/ssm/step.py`` (ported from
...
``ssm_applies`` so a caller imports from one place; the strategy form of the
same dispatch is ``FusedSsm``.
...
from mlx_omnia.engine.core.kernels.ssm.kernel import compute_dt as _compute_dt
from mlx_omnia.engine.core.kernels.ssm.step import ssm_step as _ssm_step
from mlx_omnia.engine.core.kernels.ssm.step import ssm_step_applies as _ssm_step_applies
...
def ssm_step(
    ...
    """One-token SSD step via the Metal kernel. Delegates to ``ssm.step.ssm_step``.
