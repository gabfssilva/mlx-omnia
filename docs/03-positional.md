...
**Traditional vs half-split pairing.** Which components form a pair — `(0,1), (2,3), …` (interleaved, "traditional") or `(0, D/2), (1, D/2+1), …` (split-half). Mathematically equivalent under a permutation of the weight columns; *not* interchangeable against a given checkpoint, whose weights were trained under one of them. A `traditional` flag threads through `FusedQKVAttention.rope`.
...
**NTK-by-parts (YaRN, and the llama3 formula).** Do neither uniformly. Classify each pair by how many full rotations it completes over the original context:
