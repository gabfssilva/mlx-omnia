...
The operation uses three projections. `gate` and `up` both have shape `[hidden, inner]`; `down` returns to `[inner, hidden]`. `⊙` is elementwise multiplication. With `act = swish`, the result is **SwiGLU**, used by nearly every current model. Gemma puts tanh-approximated GELU in the same slot. The gate controls how much of the other branch passes for each channel and token.
...
Total parameters scale with `E`; work per token scales with `k`. A 30B model with 8 of 128 experts active does the arithmetic of roughly a 3B model. This separation between capacity and active compute explains why recent frontier open-weight models are usually sparse.
...
**Scoring: softmax or sigmoid.** Softmax over all `E` experts makes the surviving weights depend on the ones that lost: change an unselected expert and the kept weights change. Sigmoid scores each expert independently, which decouples selection and weight. A port must reproduce the checkpoint's choice.
...
The experts are stored as one stacked tensor per projection, `[E, out, in]` (`core/layers.py::SwitchLinear`). Applying the selected expert becomes a *gather* fused with the matmul: select rows of the stack by index.

- `mx.gather_mm` for dense weights,
- `mx.gather_qmm` for quantized ones (chapter 06).

`SwitchGLU` stacks gate and up **row-interleaved** on the output axis: `[g₀, u₀, g₁, u₁, …]`. A single gather reads both, and each `(gate, up)` pair lands adjacent for the decode kernel. This applies the dense `SwiGLU` fusion one dimension higher.
...
Sparsity changes the cost model in four places.
...
**Speculative decoding pays for the routing union.** A round that verifies `k + 1` rows gathers the union of their selected experts. A dense target reads its weights once per round; a sparse target's read grows with the number of rows verified. `omnia-bench interleaved` reports two bounds for this case because the harness cannot measure the routing union.
