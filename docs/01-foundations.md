...
Everything else — chat, tools, JSON output, agents — is built on repeated application of that one function. The model itself has no notion of a conversation; a conversation is a string that a template produced, tokenized.
...
- **Embedding**: a `[vocab, hidden]` table. Token id `i` selects row `i`. A gather, not a matmul.
...
Tying is not a detail. It halves the largest single tensor in a small model, and it changes what a decode step has to read — see chapter 07.
...
Two properties matter and both come from the `x +`. The residual stream is a single vector per position that every block reads and writes into, which is why `hidden_size` is constant through the trunk. And gradients — during training — reach layer 0 without passing through `L` multiplications, which is why the stack can be deep at all.
...
Models do not see text. They see ids from a fixed vocabulary, produced by a byte-level BPE tokenizer whose full description is in the checkpoint's own `tokenizer.json` (`bpe.py`).
...
Generation is not one loop, it is two regimes with completely different cost structure.
...
Constrained decoding — forcing output to match a grammar or JSON schema — is not a sampler. It is a mask over logits that depends on what was drawn *before*, so it has state, and it lives behind the `Constraint` protocol in `generate.py` with the implementation in `grammar.py`.
