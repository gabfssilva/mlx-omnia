"""Qwen3.5 under continuous batching: a batch of rows matches the rows decoded alone.

Tiny randomized weights, no checkpoint: what is under test is that the ragged batch path
(`BatchedKVCache` for the full-attention layers, `BatchedDeltaCache` for the DeltaNet ones)
...
def test_rows_are_isolated() -> None:
    ...
    through its KV, the DeltaNet layer through its recurrent state and its conv window."""
    model = tiny_model()
    batched = [model.make_cache() for _ in PROMPTS]
