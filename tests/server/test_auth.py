...
function of the path prefix and the config table, `create_app`'s line is the wiring and not
the policy, and every stub answers `{"ok": true}` — so a 200 here is the middleware letting
the request through and nothing else.
...
the config once at boot would answer 200 to all of them.
"""
...
from mlx_omnia.server.api import auth
...
def build() -> FastAPI:
    """The stub app, over the one database the daemon reads: the middleware asks
    `services.config`, so the lifespan has to migrate and connect the same way `main` does."""
...
def test_a_key_closes_every_route_but_health(client: TestClient) -> None:
...
def test_the_openai_dialect_is_refused_in_its_own_envelope(client: TestClient) -> None:
    set_key(KEY)
...
def test_the_anthropic_dialect_is_refused_in_its_own_envelope(client: TestClient) -> None:
...
def test_the_gemini_dialect_is_refused_in_its_own_envelope(client: TestClient) -> None:
...
def test_admin_is_refused_in_the_envelope_the_rest_of_admin_uses(client: TestClient) -> None:
    """OpenAI's, which is what `main.py`'s validation handler already answers with on every
...
def test_each_dialect_carrier_opens_the_door(client: TestClient) -> None:
...
def test_a_bare_authorization_header_is_not_a_key(client: TestClient) -> None:
...
def test_a_non_ascii_key_is_comparable(client: TestClient) -> None:
...
def test_a_rotation_counts_from_the_next_request(client: TestClient) -> None:
...
def test_an_unrouted_path_is_refused_before_it_is_routed(client: TestClient) -> None:
...
def test_the_middleware_reaches_the_app_the_daemon_serves() -> None:
    ...
    daemon open. Both routes answer out of the config alone — a request that reached a handler
...
def test_the_loopback_binds_without_a_key() -> None:
    for host in ("127.0.0.1", "127.0.0.2", "::1", "localhost"):
        auth.check_bind(host, None)


def test_a_bind_off_the_loopback_without_a_key_is_refused() -> None:
...
def test_a_bind_off_the_loopback_with_a_key_comes_up() -> None:
    auth.check_bind("0.0.0.0", KEY)
