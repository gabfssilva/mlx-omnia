...
from mlx_omnia.server.runtime.engine import ModelTooLarge
from tests.server.admission_stand import (
    _DEADLINE,
    _LATENESS,
    _SLACK,
    _TTL_SECONDS,
...
def test_the_load_that_crosses_the_ceiling_evicts_the_least_recently_used(hub: Path) -> None:
    ...
    async def run() -> list[dict[str, object]]:
        ...
        async with serving(lambda _: weighted()) as (http, engine):
...
def test_a_model_larger_than_the_whole_ceiling_is_refused_and_evicts_nothing(hub: Path) -> None:
...
def test_the_idle_ttl_expires_and_the_weights_actually_come_back(hub: Path) -> None:
...
def test_a_queued_request_keeps_its_model_through_the_pressure(hub: Path) -> None:
    ...
    async def run() -> tuple[list[dict[str, object]], list[Segment], list[Segment]]:
        for model_id in (FIRST, SECOND, THIRD):
            installed(hub, model_id)
        async with serving(lambda _: gated(gate)) as (http, engine):
