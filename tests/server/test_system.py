...
def test_the_version_is_the_distribution_s(payload: dict[str, object]) -> None:
    """There is one distribution now, so there is no engine-versus-server version left to
    tell apart, and no manifest to read a literal out of — `hatch-vcs` writes the number
    from the tag at build time and nothing in the tree restates it. What survives is the
    narrow thing: the route reports installed metadata rather than a string of its own.
    """
    assert payload["version"] == version("mlx-omnia")
