"""LFM2-MoE under continuous batching: a batch of rows matches the rows decoded alone.
...
from mlx_omnia.engine.models.lfm2.config import LFM2MoEConfig, LFM2RoPEParameters
from mlx_omnia.engine.models.lfm2.moe.model import LFM2MoE


def tiny_model() -> LFM2MoE:
    mx.random.seed(11)
    model = LFM2MoE(
        LFM2MoEConfig(
            hidden_size=32,
...
def test_batched_rows_match_solo_rows() -> None:
    ...
    for _ in range(4):
        ...
        for index, row in enumerate(rows):
            ...
            # 1e-3 and not the template's 1e-4: measured order-dependent kernel
            # resolution noise — 8e-6 absolute over a ~0.02 ceiling when the MoE
            # delegators resolve after another suite already wired kernels.
            assert difference / ceiling < 1e-3, f"row {index} diverged"
