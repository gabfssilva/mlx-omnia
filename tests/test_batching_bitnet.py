"""BitNet under continuous batching: a batch of rows matches the rows decoded alone.
...
def tiny_model() -> BitNet:
    ...
    # Only the floating parameters: BitNet packs ternary weights as uint8 fields, and a
    # normal draw in their place is not a weight at all.
    model.update(
        tree_map(
            lambda p: mx.random.normal(p.shape) * 0.05
            if p.dtype in (mx.float32, mx.float16, mx.bfloat16)
            else p,
            model.parameters(),
        )
    )
