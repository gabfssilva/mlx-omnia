...
The shared spine (``tests/parity/definition.py``) carries the trunk floors and the greedy
match. The cache tests stay here: their bar is the fixture's measured ``noise.batching``,
not the spine's fp32 1e-5.
...
from tests.parity.definition import a_parity_trunk, an_exact_embedding_lookup
...
@behaves_like(a_parity_trunk, an_exact_embedding_lookup)
def describe_bitnet():
    ...
    def it_holds_the_sub_norms_within_floor(model: BitNet, golden: dict[str, mx.array]) -> None:
    ...
    def it_agrees_with_prefill_stepwise(model: BitNet, golden: dict[str, mx.array]) -> None:
    ...
    def it_replays_the_fixture_greedy_run(model: BitNet, golden: dict[str, mx.array]) -> None:
    ...
    def it_fails_when_a_weight_scale_is_perturbed(
        model: BitNet, golden: dict[str, mx.array]
    ) -> None:
        ...
        down = model.model.layers[13].mlp.down_proj
        tripled = down.weight_scale * mx.array([3.0], dtype=down.weight_scale.dtype)
        with mutated(down, "weight_scale", tripled):
